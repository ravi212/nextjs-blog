...
const Home = ({posts}: {posts: PostType[]}) => {
    ...
    const featuredPosts = posts.filter((post: PostType) => post?.featured && !post?.pinned);
    // @ts-ignore
    const recentPosts = posts.sort((a,b) => new Date(a.updatedAt) - new Date(a.updatedAt))
  ...
  return (
    ...
    <Divider />
    {
        featuredPosts && <div className='grid md:grid-cols-2 grid-cols-1 gap-4'>
            {
    ...
    <div className='flex justify-between sm:flex-row flex-col py-6 px-3'>
        ...
        <Link href={`/category/all`} className='text-center text-sm text-blue-500 underline'>View All</Link>
    </div>
    ...
    {/* recent posts */}
    {
        recentPosts && <div className='grid grid-cols-2 gap-4'>
            {
